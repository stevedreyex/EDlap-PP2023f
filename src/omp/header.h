...
void stencilCodeOMP( Image* in, Image* out ) {
	double* buffer = (double*)calloc(in->width * in->height * 3,sizeof(double));
	double min = 1.0, max = 0.0;
...
Image* image_load( const char *file_name ) {
	...
	png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL);	//the last three are error handler functon
