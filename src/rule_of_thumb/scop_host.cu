...
#define cudaCheckKernel() \
  do { \
    cudaCheckReturn(cudaGetLastError()); \
      ...
      cudaCheckReturn(cudaMalloc((void **) &dev_buffer, (36000000) * sizeof(double)));
      cudaCheckReturn(cudaMalloc((void **) &dev_out, (6000) * (6000) * sizeof(int)));
      
      cudaCheckReturn(cudaMemcpy(dev_buffer, buffer, (36000000) * sizeof(double), cudaMemcpyHostToDevice));
      {
        dim3 k1_dimBlock(16, 32);
        dim3 k1_dimGrid(256, 188);
        kernel1 <<<k1_dimGrid, k1_dimBlock>>> (dev_buffer, max, min, dev_out);
        cudaCheckKernel();
      }
      
      cudaCheckReturn(cudaMemcpy(out, dev_out, (6000) * (6000) * sizeof(int), cudaMemcpyDeviceToHost));
      cudaCheckReturn(cudaFree(dev_buffer));
